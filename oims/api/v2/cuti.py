    ...
    try:
        ...
        for cuti in data:
            ...
            approval_rows = frappe.get_all(
                "Cuti Tahunan Approval",
                filters={"parent": cuti["name"], "parenttype": "Cuti Tahunan"},
...
def create_cuti_tahunan(keperluan, karyawan_penerima_job_pending, tanggal_mulai_cuti, tanggal_selesai_cuti):
    try:
        user_id = frappe.session.user

        karyawan_name = frappe.db.get_value(
            'Karyawan',
            {'user_id': user_id},
            'name'
        )
        ...
        doc = frappe.get_doc({
            "doctype": "Cuti Tahunan",
            "karyawan_pemohon": karyawan_name,
        ...
        return response_success("Cuti tahunan berhasil dibuat.", data={"cuti_id": doc.name})

    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Error create_cuti_tahunan")
        return response_error(
            "Terjadi kesalahan saat membuat data cuti tahunan.",
...
def get_cuti_tahunan_detail(name) -> dict:
    try:
        ...
        cuti = frappe.get_value(
            "Cuti Tahunan",
            name,
            ["name", "karyawan_pemohon", "keperluan", "karyawan_penerima_job_pending", "tanggal_mulai_cuti", "tanggal_selesai_cuti", "selama_hari", "creation"],
            as_dict=True
        )

        if not cuti:
            return response_error("Data cuti tahunan tidak ditemukan.", http_status_code=404)
        ...
        # --- Timeline Logic ---
        timeline = []
        # 1. Selalu ada pengajuan cuti tahunan
        timeline.append({
        ...
        return response_success("Berhasil mengambil detail cuti tahunan.", data=cuti)
    except Exception as e:
...
def get_approval_rights(name):
    ...
    try:
        ...
        # Ambil approval rows untuk cuti tahunan ini
        approval_rows = frappe.get_all(
            "Cuti Tahunan Approval",
            filters={"parent": name, "parenttype": "Cuti Tahunan"},
...
def sign_cuti_tahunan_approval(name, action=None):
    """
    User login menandatangani approval pada dokumen Cuti Tahunan (child table).
    ...
    try:
        ...
        doc = frappe.get_doc("Cuti Tahunan", name)
        ...
        if next_approval:
            if action == "approve":
                ...
                frappe.get_doc({
                    ...
                    "title": "Tanda Tangan Pengajuan Cuti Tahunan",
        ...
        return response_success("Berhasil menandatangani approval cuti tahunan.", data={"name": name})
...
def is_employee_allow_to_make_cuti_tahunan() -> dict:
    try:
        ...
        is_allowed = all([
            bool(karyawan.get('jabatan')),
            bool(karyawan.get('nrp')),
            bool(karyawan.get('divisi'))
        ])

        return response_success(
            "Berhasil cek detail cuti tahunan.",
            data=is_allowed
        )
    except Exception as e:
