...
class SuratTugas(Document):
    def before_save(self):
        ...
        nama_surat_pdf = f"{nama_surat_raw}.pdf"
        penandatangan = frappe.get_doc('Karyawan', self.penanda_tangan)

        generated_docx = self.generate_document(nama_surat_temp, karyawan_items, penandatangan)
        self.upload_document_to_file_manager(nama_surat, generated_docx)

        generated_pdf = self.convert_docx_to_pdf(generated_docx, nama_surat_pdf)
        self.upload_document_to_file_manager(nama_surat_pdf, generated_pdf)
    ...
    def upload_document_to_file_manager(self, nama_surat, file):
        try:
            ...
            if file.endswith('.pdf'):
                self.pdf_url = file_url  # Assumes 'pdf_url' field exists on the Surat Tugas doctype
            else:
                self.doc_url = file_url  # Assumes 'doc_url' field exists on the Surat Tugas doctype
    ...
    # def convert_docx_to_pdf(self, docx_file_path, pdf_file_name):
    #     try:
    #         pdf_file_path = frappe.utils.get_site_path('private', 'files', pdf_file_name)
    #         pypandoc.download_pandoc()  # Download pandoc if not already installed
    #         pypandoc.convert_file(docx_file_path, 'latex', outputfile=pdf_file_path)
    #         print(f"File converted to PDF: {pdf_file_path}")
    #         return pdf_file_path
    #     except Exception as e:
    #         print(f"Error converting file to PDF: {e}")
    #         frappe.logger().error(f"Error converting file to PDF: {e}")
    #         return None

    def convert_docx_to_pdf(self, docx_file_path, pdf_file_path):
        try:
            subprocess.run(["libreoffice", "--headless", "--convert-to", "pdf", docx_file_path, "--outdir", pdf_file_path.rsplit("/", 1)[0]], check=True)
            print(f"File converted to PDF: {pdf_file_path}")
            return pdf_file_path
