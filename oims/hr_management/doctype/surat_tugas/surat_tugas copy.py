...
class SuratTugas(Document):
	def before_save(self):
		try:
			# Force error
			a = 1 / 0  # Ini akan raise ZeroDivisionError
			frappe.throw("This is a test error", title="Test Error")
			...
			frappe.log_error("Error in before_save", frappe.get_traceback())
			frappe.logger("SuratTugas").error(f"Error in before_save: {e}")
		...
		try:
			...
			for row in self.karyawan:
				if row.employee_id and row.employee_id in karyawan_map:
					...
					karyawan_doc['tanggal_beransdagkat'] = tanggal_berangkat
			...
			frappe.logger().error(f"Error in validate_karyawan_data: {e}")
	...
	def generate_document(self, nama_surat, karyawan_items, penandatangan):
		try:
			...
			frappe.logger().error(f"Error in generate_document: {e}")
	...
	def generate_single_document(self, doc, karyawan_items, tanggal, nama_surat, lokasi_site_formatted, penandatangan):
		try:
			...
			context = {
				...
				'tanggal': tanggal,
				'nama': karyawan_items[0]['nama_lengkap'],
			...
			frappe.logger("SuratTugas").error(frappe.get_traceback())
			return None
